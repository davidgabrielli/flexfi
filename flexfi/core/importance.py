...
def feature_importance_statistics(
    ...
    pd.DataFrame
        ...
        ['Feature/Group', 'Importance Score', 'Importance Score STD', 'P-Value']
    """
...
# ---------------------------------------------------------------------
# Feature Importance Scoring (Global Permutation + Streaming Batches)
# ---------------------------------------------------------------------
def feature_importance_scores_batch(
    model: nn.Module,
    outcome: str,
    ...
    """
    Computes per-sample feature importance scores using global permutations
    while processing the dataset in batches (memory-efficient).
    ...
    # Normalize feature_indices so it's always a list
    if isinstance(feature_indices, int):
        feature_indices = [feature_indices]
    elif isinstance(feature_indices, (tuple, set)):
        feature_indices = list(feature_indices)
    elif isinstance(feature_indices, list) and any(isinstance(i, list) for i in feature_indices):
        # Flatten nested lists if user passed [[0,1,2]]
        feature_indices = [i for sub in feature_indices for i in sub]
    ...
    with torch.inference_mode():
        # Compute original per-sample losses batch by batch
        orig_losses = torch.zeros(n_samples, device=device)
        start_idx = 0
        for X_batch, y_batch in data_loader:
            ...
            preds = model(inputs)
            orig_loss = loss_fn(preds, labels).squeeze()
            orig_losses[start_idx:end_idx] = orig_loss

            start_idx = end_idx

        # Compute permuted losses (one permutation at a time)
        perm_loss_all = torch.zeros((n_samples, perm_count), device=device)

        for p in range(perm_count):
            # Apply a global permutation for this feature or group
            X_perm = permute_input_tensor(X_full, feature_indices)

            # Evaluate permuted data batch by batch
            start_idx = 0
            for X_batch, y_batch in data_loader:
                batch_size = X_batch.shape[0]
                end_idx = start_idx + batch_size

                inputs_perm = X_perm[start_idx:end_idx].float()
                ...
                preds_perm = model(inputs_perm)
                perm_loss = loss_fn(preds_perm, labels).squeeze()
                perm_loss_all[start_idx:end_idx, p] = perm_loss

                start_idx = end_idx

    # Compute mean permuted loss and FI scores
    mean_perm_loss = perm_loss_all.mean(dim=1, keepdim=True)
    returnscore = orig_losses.unsqueeze(1) - mean_perm_loss
    returnscorelist = orig_losses.unsqueeze(1) - perm_loss_all

    return returnscore.squeeze(), returnscorelist
...
def feat_imp_per_unit(
    ...
    This function trains a new model on each fold of the data, evaluates model metrics, 
    and evaluates the importance of the specified feature or group  using permutation-based 
    feature importance on the held-out test set.
    ...
    for fold, (train_ids, valid_ids) in enumerate(kfold.split(full_dataset)):
        ...
        model_fold = init_weights(model_fold, init_method)
        
        # Set up optimizer and scheduler
        optimizer = create_optimizer(model_fold, optimizer_type, learning_rate, weight_decay)
        ...
        for epoch in range(epochs):
            model_fold.train()
            optimizer.zero_grad()
            outputs = model_fold(inputs_train)
            loss = loss_fn_mean(outputs, labels_train)
            
            # Apply regularization penalty if specified
            loss = apply_penalty(model_fold, input_size, penalty_type, loss)
        ...
        # Compute metrics
        train_metrics = compute_metrics(model_fold, train_loader, outcome)
        valid_metrics = compute_metrics(model_fold, valid_loader, outcome)

        # Accumulate model metric results across folds        
    ...
    # Aggregate model performance metrics across folds
    def average_metrics(metric_list: List[Dict[str, float]]) -> Dict[str, float]:
        """Averages a list of metric dicts, ignoring missing keys."""
        all_keys = set().union(*metric_list)
        return {
            k: np.nanmean([m.get(k, np.nan) for m in metric_list])
            for k in all_keys
        }

    avg_train_metrics = average_metrics(train_metrics_all)
    avg_valid_metrics = average_metrics(valid_metrics_all)
    
    return fi_statistics_per_iteration, avg_train_metrics, avg_valid_metrics
...
def feature_importance(
    ...
    dict
        ...
        - "model_metrics_summary": DataFrame with overall average train/validation AUC and accuracy
    ...
    for iteration in range(iterations):
        ...
        for unit_name, feature_indices in eval_units:
            ...
            result_df, train_metrics, valid_metrics = feat_imp_per_unit(        
                X=X, y=y, outcome=outcome, model=model,
                feature_indices=feature_indices,
                feature_names=feature_names,
            ...
            # Combine iteration-level metrics for aggregation
    ...
    # Support function to avoid printing very small numbers as 0
    ...
    # Process and format results - Model Metrics
    metrics_df = pd.DataFrame(metrics_per_iteration)
        
    # Compute overall average metrics
    model_summary = metrics_df.drop(columns=["Iteration"]).mean().to_frame().T

    # Print and save summary results - Model Metrics
    with pd.option_context('display.float_format', floor_format):
        print("\nOverall Model Performance Summary:\n")
        print(model_summary.to_string(index=False))

    model_summary.to_csv("model_metrics_summary.csv",
                         index=False, float_format="%.4g")

    # Process and format results - FI
    ...
    # Print and save summary results - FI 
    with pd.option_context('display.float_format', floor_format):
    ...
    results: Dict[str, pd.DataFrame] = {
        ...
        "model_metrics_summary": model_summary
    }        
    return results