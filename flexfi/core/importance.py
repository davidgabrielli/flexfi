...
def feature_importance_statistics(
    ...
    covariate : int
        Index of the covariate/feature being analyzed.
    column_names : list of str, optional
        List of feature names. If provided, will use actual feature names instead of 
        generic "Feature_X" labels.
    input_size : int, optional
        Total number of features. Required if column_names is provided.

    Returns:
    --------
    pd.DataFrame
        Single-row DataFrame with columns:
        ['Covariate', 'Importance Score', 'Importance Score STD', 'P-Value']
    """
    ...
    stt_std = torch.sqrt(torch.sum(torch.var(importance_score_list_all, dim=1, unbiased=False))).item() / n_samples
    ...
    # Assign feature name
    if column_names is not None and len(column_names) == input_size:
        covariate_name = column_names[covariate]
    else:
        covariate_name = f"Feature_{covariate}"
    
    # Create results DataFrame
    fi_statistics_per_iteration = pd.DataFrame(
        [[covariate_name, stt, stt_std, pval]],
        columns=['Covariate', 'Importance Score', 'Importance Score STD', 'P-Value']
    )
...
def feature_importance_scores(
    ...
    For a given feature/covariate, this function compares the model's prediction loss on 
    the original dataset versus the loss after permuting that feature's values across 
    samples. The difference in loss indicates how important that feature is for the 
    model's predictions.
    ...
    covariate : int
        Index of the feature/covariate to permute (0-indexed).
    loss_fn : callable
    ...
    for X_batch, y_batch in data_loader:
        ...
        with torch.inference_mode():
            ...
            for i in range(perm_count):
                inputs_perm = permute_input_tensor(inputs, cov_idx=covariate)
        ...
        # Positive values indicate feature importance (permuting hurts performance)
...
def feat_imp_per_cov(
    ...
    """
    Computes feature importance for a single covariate using cross-validation.

    This function trains a fresh model on each fold of the data and evaluates 
    the importance of the specified covariate using permutation-based feature 
    ...
    covariate : int
        Index of the feature to evaluate importance for.
    model_seed : int
    ...
    pd.DataFrame
        ...
        ['Covariate', 'Importance Score', 'Importance Score STD', 'P-Value', 'Iteration']
    """
...
def feature_importance(
    ...
    """
    Computes permutation-based feature importance for all features using cross-validation.

    This is the main entry point for feature importance analysis. It trains models 
    across multiple iterations (with different random seeds) and computes importance 
    scores for all features using proper cross-validation methodology.
    ...
    ValueError
        If shapes are incompatible, iterations < 1, or outcome is invalid.
    ...
    >>> results = feature_importance(X, y, "binary", iterations=3)
    ...
    for iteration in range(iterations):
        ...
        for cov_idx in range(n_features):
            print(f"Running Feature Importance for covariate {cov_idx}")
            result_df = feat_imp_per_cov(        
    ...
    fi_per_iteration.sort_values(by=["Covariate", "Iteration"], inplace=True)
    ...
    fi_summary = (fi_per_iteration
        .groupby("Covariate")[["Importance Score", "Importance Score STD", "P-Value"]]
        .mean()
        .reset_index()
    )
