...
def plot_fi_bar(
    ...
    results_df : pd.DataFrame
        ...
        - 'Covariate': Feature names or identifiers
    ...
    n_covariates = results_df["Covariate"].nunique()
...
def plot_fi_violin(
    ...
    top_features = (
        results_df.groupby("Covariate")["Importance Score"]
        .mean()
    ...
    subset = results_df[results_df["Covariate"].isin(top_features)]
    ...
    mean_scores = (
        subset.groupby("Covariate")["Importance Score"]
        .mean()
...
def plot_score_vs_pval(
    ...
    if annotate:
        ...
        top_features = results_df.sort_values("Importance Score").head(top_n)
        texts = [
            ax.text(row["Importance Score"], row["P-Value"], row["Covariate"],
